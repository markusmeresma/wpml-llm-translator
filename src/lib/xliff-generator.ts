...
export function isAutoGeneratedExcerpt(
  bodyHtml: string,
  excerptText: string
): boolean {
  const trimmedExcerpt = excerptText
    .replace(/\[\s*(?:\u2026|\.{3}|&hellip;)\s*\]$/, "")
    .trim();
  ...
  const excerptPlain = stripHtmlToText(trimmedExcerpt);

  if (!excerptPlain) {
    return false;
  }

  return bodyPlain.startsWith(excerptPlain);
}
...
export function generateProductXliff(input: GenerateProductXliffInput): string {
  ...
  if (excerpt.trim() && !isAutoGeneratedExcerpt(body, excerpt)) {
